...
boolean Adafruit_FONA::MMS_initialization(void){
  getReply(F("AT+CMMSCURL=\"mms.fido.ca\""));
  getReply(F("AT+CMMSCURL?"));
  getReply(F("AT+CMMSPROTO=1,\"205.151.11.13\",80"));
  getReply(F("AT+CGSOCKCONT=1,\"IP\",\"fido-core-appl1.apn\""));

}

boolean Adafruit_FONA::MMS_sendMMS(char *smsaddr){
  if (!SD.begin(10)) {
  ...
  if (SD.exists("a.jpg")) {
    myFile = SD.open("a.jpg");
    //while(myFile.
  } else {
    Serial.println("a.jpg doesn't exist.");
  }

  int remainBytes = myFile.size();

  getReply("AT+CMMSEDIT=1");
  getReply("AT+CMMSRECP=\"yyx3636@gmail.com\"");
  //getReply("AT+CMMSDOWN=\"FILE\",0,\"test.jpg\"");
  //getReply("AT+CMMSDOWN=\"PIC\",11280,\"a.jpg\"");

  mySerial->print("AT+CMMSDOWN=\"PIC\",");
  mySerial->print(remainBytes);
  mySerial->println(",\"a.jpg\"");
  ...
  while (remainBytes > 0){
    ...
    len = min(len,remainBytes);
    remainBytes -= len;

    //Serial.println(remainBytes);
    myFile.read(buf,len);
    //Serial.println(buf);

    sendCheckReplyB(buf,len,"");
  }

  //delay(1000);

  //mySerial->println();
  //getReply("AT+CMMSSAVE=0,1");
  getReply("AT+CMMSSEND");

  Serial.println("All done");
  //return sendCheckReply("",F("+CMMSSEND: 0"),10000);
  return true;
}
/********* file system ****************************************************/
...
boolean Adafruit_FONA::TEST_2(char * i){
  //  getReply(F("AT+CMMSSAVE=1,0"));
  //  getReply(F("AT+CMMSVIEW"));
  //  getReply(F("AT+CMMSVIEW=1"));
  //  getReply(F("AT+CMMSSNATCH=0,0,\"test.jpg\""));

  //getReply(F("AT+FSLS=0"));
